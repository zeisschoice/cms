	...
	//	room.setRoomName("测试用的");
	 ...
	 public Object selectLastMon(Cost cost) {
		// iCostService.sele
		 ...
		 Cost rs = iCostService.selectById(cost.getId());
	 ...
	 @ResponseBody
	 public Object getCurMonPay() throws InstantiationException, IllegalAccessException {
		...
		// int payRooms = iCostService.selectCount(cost);
		 ...
		 EntityWrapper<Cost> wrapper = new EntityWrapper<Cost>();
		 
		
		 wrapper.eq("year", year);
		 wrapper.eq("is_pay",1);
		 wrapper.eq("month",month + 1);
	
		 
		 
		 int payRooms = iCostService.selectCount(wrapper);
